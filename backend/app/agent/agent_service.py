    ...
    async def process_message(
        ...
        while call_count < max_calls and not final_answer:
            ...
            # Tool execution request
                ...
                messages.append({
                    "role": "assistant",
                    "content": json.dumps({"action": "tool", "tool": tool_name, "arguments": arguments}, default=str)
                })
                messages.append({
                    "role": "user",
                    "content": f"TOOL EXECUTION RESULT for '{tool_name}': {json.dumps(tool_result, default=str)}. Now explain these findings clearly to the user in a final response."
                })
